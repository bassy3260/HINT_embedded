...
URL = 'http://192.168.0.26:9999/'  # 서버 주소
get = lambda name: urllib.request.urlopen(URL + name).read()  # 서버에서 파일 하나 받아오는 함수 (예: get('A.enc'))

# 1) 받기
open('pub.pem', 'wb').write(get('public.pem'))  # 서버 공개키 받아서 파일로 저장 (openssl 이 파일로 읽어야 해서)
key_enc = get('key.enc')  # 개인키로 암호화된 AES 키 받기
enc = get('firmware.enc')  # 암호화된 파일 A 받기
manifest = get('firmware.sha256').decode().strip()  # 해시가 적힌 manifest 받아서 딕셔너리로 변환

# 1.5) 서명 검증   ← 여기부터 새로 추가
open('firmware.sha256', 'wb').write(get('firmware.sha256'))
open('firmware.sha256.sig', 'wb').write(get('firmware.sha256.sig'))
try:
    ssl('dgst', '-sha256', '-verify', 'pub.pem', '-signature', 'firmware.sha256.sig', 'firmware.sha256')
except subprocess.CalledProcessError:
    raise SystemExit('서명 불일치 → 폐기')
...
# 3) 공개키로 AES 키 복원
key = ssl('pkeyutl', '-verifyrecover', '-pubin', '-inkey', 'pub.pem', '-pkeyopt', 'rsa_padding_mode:none', data=key_enc)[-32:]  # 패딩 없이 풀고 뒤 32바이트 = AES 키
